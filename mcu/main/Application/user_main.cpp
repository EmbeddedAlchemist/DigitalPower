...
extern "C" void user_main(void *unused_arg) {
    ...
    bool h = true;
    while(true) {
        temperature_sensor_mosfet.update_loop_handle();
        console.handle_send();
        // console.debug("tempurature: %f", temperature_sensor_mosfet.get_temperature());
        osThreadYield();
        if(HAL_GetTick() - t > 3000){
            t = HAL_GetTick();
            h = !h;
            pd.set_voltage(h ? 10 : 15);
        }
    }
}
